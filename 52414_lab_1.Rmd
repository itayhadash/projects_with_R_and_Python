...
recovered$Date <- as.Date(recovered$Date)
...
rownames(recovered.agg) <- 1:nrow(recovered.agg)
cases.agg <- cases.agg[order(cases.agg$Country.Region),]
rownames(cases.agg) <- 1:nrow(cases.agg)
deaths.agg <- deaths.agg[order(deaths.agg$Country.Region),]
rownames(deaths.agg) <- 1:nrow(deaths.agg)

recovered.agg_tidy <-as.data.frame( recovered %>% group_by(Country.Region,Date) %>% summarise(Value = sum(Value)))
# murders %>% group_by(region) %>% summarise(n= n(),mean = mean(total))
cases.agg_tidy <-as.data.frame( cases %>% group_by(Country.Region,Date) %>% summarise(Value = sum(Value)))
deaths.agg_tidy <-as.data.frame( deaths %>% group_by(Country.Region,Date) %>% summarise(Value = sum(Value)))

identical(deaths.agg,deaths.agg_tidy)
identical(recovered.agg,recovered.agg_tidy)          
identical(cases.agg,cases.agg_tidy)

march_recovered <- recovered.agg[which(recovered.agg$Date=="2020-03-31"),]
march_deaths <- deaths.agg[which(deaths.agg$Date=="2020-03-31"),]
march_cases <- cases.agg[which(cases.agg$Date=="2020-03-31"),]

march <- left_join(march_cases,march_recovered,c("Country.Region","Date"))
march <- left_join(march,march_deaths,c("Country.Region","Date"))

march_top<- head(march[order(march$Value.x,decreasing = TRUE),],10)
colnames(march_top)[c(3:5)] <- c("cases","recovered","deaths")


dat <- rbind(as.vector(march_top$cases)-as.vector(march_top$deaths)-as.vector(march_top$recovered),as.vector(march_top$recovered),as.vector(march_top$deaths))

barplot(dat,ylim = c(0,200000),names.arg = as.vector(march_top$Country.Region),ylab = "Number of total cases",las=2,cex.axis = 0.7,cex.names = 0.7,col = c("yellow","green","red"),legend.text = (c("sick","recovered","death")))
...
top_diff_cases <- head(cases.agg[order(cases.agg$Diff,decreasing = TRUE),],10)
top_diff_deaths <-head(deaths.agg[order(deaths.agg$Diff,decreasing = TRUE),],10) 

print(top_diff_cases)
print(top_diff_deaths)

italy_cases <- cases.agg[which(cases.agg$Country.Region=="Italy"),]
italy_deaths <-deaths.agg[which(deaths.agg$Country.Region=="Italy"),]

plot(italy_cases$Date,italy_cases$Diff,type = "p",col ="green",lwd =3,main = "daily change in deaths and cases",ylab = "number of deaths or cases",xaxt ="n",xlab = "",cex.lab = 1,ylim = c(0,6500),las=2,)

axis(1,at=seq(italy_cases$Date[1],by = "week",length.out = 15) ,labels =seq(italy_cases$Date[1],by = "week",length.out = 15),las =2,cex.axis =0.7 )
points(italy_cases$Date,italy_deaths$Diff,col="red",lwd=3)
legend(italy_cases$Date[1],5000,legend = c("new deaths","new cases"),col=c("red","green"),lty = 1:2)


plot(italy_cases$Date,italy_cases$Diff +1,type ="p",col ="green",lwd =3,main = "daily change in deaths and cases log",ylab = "",xaxt ="n",xlab = "",log = "y",las=2)

axis(1,at=seq(italy_cases$Date[1],by = "week",length.out = 15) ,labels =seq(italy_cases$Date[1],by = "week",length.out = 15),las =2,cex.axis =0.7 )
points(italy_cases$Date,italy_deaths$Diff+1,col="red",lwd=3)
legend(italy_cases$Date[1],5000,legend = c("new deaths","new cases"),col=c("red","green"),lty = 1:2)
